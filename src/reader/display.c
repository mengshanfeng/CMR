...
void open_data_file(lbm_data_file_t * file,const char * fname)
{
	...
	//allocate memory
	file->entries = malloc(file->header.mesh_max_height * file->header.mesh_max_width * sizeof(lbm_file_entry_t));
}
...
bool read_next_frame(lbm_data_file_t * file)
{
	...
	//load the frame
	res = fread(file->entries,sizeof(lbm_file_entry_t),file->header.mesh_max_height * file->header.mesh_max_width,file->fp);

	if (res != file->header.mesh_max_height * file->header.mesh_max_width)
	{
		if (feof(file->fp))
		{
			return false;
...
{
	int res = fseek(file->fp,frame * sizeof(lbm_file_entry_t) * file->header.mesh_max_height * file->header.mesh_max_width,SEEK_CUR);
	return (res == 0);
}
...
/*******************  FUNCTION  *********************/
void print_current_frame_vtk(lbm_data_file_t * file)
{
	...
	printf("DIMENSIONS %d %d 1\n",file->header.mesh_width,line_height);
	printf("ORIGIN 0 0 0\n");
...
int get_frame_count(lbm_data_file_t * file)
{
	...
	if (fstat(fileno(file->fp), &info) == 0)
		return info.st_size / (file->header.mesh_max_height * file->header.mesh_max_width * sizeof(lbm_file_entry_t));
	else
		return 0;
}
