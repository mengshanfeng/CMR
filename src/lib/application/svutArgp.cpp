...
**/
void svutArgp::declareOption(char key, std::string name, std::string valueType, std::string descr) throw (svutExArgpDuplicateKey)
{
	svutArgDef arg;
	arg.key = key;
	arg.name = name;
...
{
	this->declareOption('?',"help","NONE","Give this help list");
	this->declareOption(SVUT_USAGE_KEY,"usage","NONE","Give a short usage message");
}
