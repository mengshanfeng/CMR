...
#ifndef CMR_MESH_OPERATION_SIMPLE_LOOP_IN_PLACE_H
...
/*******************  FUNCTION  *********************/
template <class T,class U>
CMRMeshOperationSimpleLoopInPlace<T,U>::~CMRMeshOperationSimpleLoopInPlace (void)
{
	if (action != NULL) 
		delete action;
}
...
/*******************  FUNCTION  *********************/
template <class T,class U>
void CMRMeshOperationSimpleLoopInPlace<T,U>::runOmpLoops ( CMRVarSystem * sys, const CMRRect & zone )
{
	//errors
	//assume(domainIn->isFullyInDomainMemory(zone),"Invalid zone not fully in domain.");
	//assume(domainOut->isFullyInDomainMemory(zone),"Invalid zone not fully in domain.");
